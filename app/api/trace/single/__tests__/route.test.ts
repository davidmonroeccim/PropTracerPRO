...
describe("POST /api/trace/single — the Tier 1 bulk queue is live work too", () => {
  it("refuses to touch a row the Tier 1 bulk queue is working", async () => {
    ...
    // is written to fence. That is exactly the class lesson L-015 exists to stop.
    //
    ...
    const res = await post({ ...BODY, owner_name: "Jane Smith" });
    ...
    expect(H.rpcCalls.filter((c) => c.fn === "deduct_wallet_balance")).toEqual([]);
  });
});
