...
9. **WITHDRAWN 2026-09-22 by spec D32. Do NOT build this.** This line used to say Phase 2 would turn a dossier-contacts fallback on for the Tier 2 cron. The dossier's own contacts are NEVER used, on any surface: the dossier identifies the owner and whether it is an individual or an entity, contacts come only from the separate Tracerfy or FastAppend call, and a vendor miss is a true null. `dossierContactsFallback`, `contactsNameVerified` and `TraceResult.name_verified` were removed in Task 6b; nothing is labelled not-name-verified anywhere. See lesson L-029.
10. A known key collision, the same class as spec 6.3 rule 3, RESHAPED by spec D36 (2026-09-22): the address key now needs BOTH a street and a city, so a record with a city and no street keys on its parcel when it has one, and `||STATE` when it does not. Street-less companies in one state therefore share `||STATE` (under D25 each new owner re-runs on it). Recorded, not solved. Phase 2 must also align the bulk surfaces, which still key with `normalizeAddress`, or the same record sent through single and bulk lands on two rows.
