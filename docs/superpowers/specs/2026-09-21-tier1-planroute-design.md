...
| D36 | (Added 2026-09-22, Task 10 review.) Amends D9 and spec 6.3. The address duplicate key is used only when a record has BOTH a street and a city; a record with a parcel id and county but no street is keyed `APN|<parcel>|<COUNTY>|<STATE>`; anything else keeps today's street-and-state key. Reason: an API record with a city, a parcel id and no street (a shape that could not reach the app before Phase 1) keyed to `|CITY|STATE`, so every such parcel in one city shared one row: a paid result was overwritten and a resend inside 90 days charged again. No existing key changes (every stored row has a street). David chose "Key on the parcel (Recommended)". |
| D37 | (Added 2026-09-22, Task 10 review.) Of three stale API-docs passages put to David, he approved only the pricing card: "Results are kept for 90 days. Resubmitting an address you already traced, with the same owner name, returns your stored result and costs nothing. A different owner name, or an owner trace that found no contacts, is traced again, and you are charged only if contacts come back. Polling any status endpoint is free." The camelCase notes (amber box, webhook note) and the examples (cached response keys, n8n comment) stay exactly as they are. |
| D38 | (Added 2026-09-22, final review.) A row keyed by parcel (`normalized_address` starting `APN|`) is never shown to a customer as that key. Every surface that displays or exports the property address shows "Parcel <parcel id>, <county> County" instead: History, the dashboard, the CSV Address column, the API status response, the MCP payloads and the contact pushed to a CRM. David chose "Parcel and county (Recommended)" over the parcel id alone. |
| D39 | (Added 2026-09-22, final review; settles todo task 20 for the single routes.) A trace that finds nothing never erases a stored result that already carries a phone or email. On that persist the row keeps its result, its owner name, its counts, its charge and its success flag; only the internal step log and contact vendor are written. The customer's response still reports this trace's own outcome, free. David chose "Keep the paid contacts (Recommended)". |
| D40 | (Added 2026-09-22, final review.) No cap on the number of owners a Full Property Trace tries: D21 stands as decided. Worst-case vendor cost per record is the dossier plus up to $0.30 for each owner named, against $0.25 or $0.40 charged. David chose "No cap (Recommended)". |
| D41 | (Added 2026-09-22, final review.) A fourth `no_lookup_key` pair, for a record sent with a parcel id but no county and no city: "This record is missing the county for that parcel ID, so it could not be looked up. You were not charged. Send it again with the county." David chose "Name the county (Recommended)". |
